...
class Streams {
    ...
    runUtil() {
        ...
        if (argv.action) {
            ...
            switch (argv.action) {
                case 'io':
                    this.inputOutput(pathToFile);
                    break;
                case 'transform':
                    this.transform();
                    break;
                case 'transform-file':
                    this.transformFile(pathToFile, false);
                    break;
                case 'transform-file-and-save':
                    this.transformFile(pathToFile, true);
                    break;
                case 'bundle-css':
                    this.cssBundler(argv.path);
                    break;
                default:
...
// EXAMPLE: npm start -- -a bundle-css -p ./assets/css
    async cssBundler(pathToFile) {
        ...
        if (stats.isDirectory()) {
            ...
            writeStream2.on('finish', () => {
                console.log('All data was written to file bundle.css');
            });
        }
    };
}
